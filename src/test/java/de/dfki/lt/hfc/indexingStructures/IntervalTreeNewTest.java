 ...
 * @version 30.10.17
 */
public class IntervalTreeNewTest extends IndexStructureTest {
    ...
    private final Map<IntervalTest, Set<int[]>> iMap = new TreeMap<>();
    ...
    protected void addInterval(AnyType start, AnyType end, Set<int[]> value) {
        IntervalTest key = new IntervalTest(start, end);
...
//    }
    ...
    private void validateIntervalSearch_Overlap(XsdInt start, XsdInt end) throws IndexingException {
        ...
        for (Map.Entry<IntervalTest, Set<int[]>> entry : iMap.entrySet()) {
            IntervalTest key = entry.getKey();
