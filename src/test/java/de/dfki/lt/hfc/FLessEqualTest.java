...
public final class FLessEqualTest {
  ...
  public void testcleanUpTuple() throws FileNotFoundException,
    ...
    // do operation
    assertEquals( FunctionalOperator.TRUE, fop.apply(ids),"0.01 <= 0.02");
    reverse(ids);
    assertEquals( FunctionalOperator.FALSE, fop.apply(ids),"0.02 <= 0.01");
    ids[0] = ids[1];
    assertEquals( FunctionalOperator.TRUE, fop.apply(ids), "0.01 <= 0.01");
  }
}
