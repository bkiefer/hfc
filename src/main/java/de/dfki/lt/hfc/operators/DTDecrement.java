 ...
 */
public class DTDecrement extends FunctionalOperator {
  @Override
  public int apply(int[] args) {
    ...
    if (date.second > 1)
      ...
      newDate = new XsdDateTime(date.year, date.month, date.day, date.hour, date.minute - 1, 59);
    else if (date.hour > 1)
      newDate = new XsdDateTime(date.year, date.month, date.day, date.hour - 1, 59, 59);
    else if (date.day > 1)
      newDate = new XsdDateTime(date.year, date.month, date.day - 1, 23, 59, 59);
