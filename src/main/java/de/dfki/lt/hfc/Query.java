...
public class Query {
   ...
   * @throws QueryParseException iff query is syntactically incorrect
   */
  public BindingTable query(String query) throws QueryParseException {
    LogMF.debug(LOGGER, "Processing query {0}", query);
    ...
    LogMF.trace(LOGGER, "SELECT successfully parsed: {0}", projectedVars);
    // parse WHERE section
    ...
    LogMF.trace(LOGGER, "WHERE successfully parsed: {0}", whereClauses);
    // some further checks on the projected indexToVariable, independent of optional filters
    if (projectedVars.contains("*")) {
      // a "*" should not come up with further indexToVariable
      if (projectedVars.size() > 1) {
        throw new QueryParseException("\"*\" and further variables can not be mixed");
      }
    } else {
      // projected indexToVariable should only consist of found indexToVariable
      HashSet<String> pv = new HashSet<String>(projectedVars);
      pv.removeAll(foundVars);
      if (!pv.isEmpty()) {
        throw new QueryParseException(
            "SELECT ontologyContainsTuple variables not found in WHERE: " + pv);
      }
    }
    ...
    if (hasFilter != -1) {
      ...
      LogMF.trace(LOGGER, "FILTER successfully parsed: {0}", filterClauses);
    }
    ...
    if (hasAggregate != -1) {
      ...
      LogMF.trace(LOGGER, "AGGREGATE successfully parsed: {0}", aggregateClauses);
    }
    //
    ...
    // further a mapping of id to QRelations is established, this is necessary as we use indexToVariable, i.e., neg ids to represent internalized relations.
    // The mapping allows us to distinguish between normal indexToVariable and indexToVariable introduced by relations
    ...
    if (Collections.EMPTY_LIST == internalizeWhere(whereClauses, patterns, lkps, nameToId, idToName,
        filterClauses, foundVars)) {
      LogMF.debug(LOGGER, "No match found for query due to unknown constant in WHERE", null);
    ...
    // note: all filter indexToVariable are definitely contained in found indexToVariable at this point
    ...
    if (hasAggregate != -1) {
      ...
      internalizeAggregate(aggregateClauses, aggregates, nameToId, idToName, projectedVars,
          foundVars);
    }
    ...
    long parsingTime = parsing - start;
    long lookupTime = end - parsing;
    LogMF.info(LOGGER, "Query processing took {0}", processTime);
    LogMF.info(LOGGER, "Parsing took {0}", parsingTime);
    LogMF.info(LOGGER, "Lookup took {0}", lookupTime);
    return bt;
  }
  ...
  private void parseFilter(StringTokenizer st,
    ...
    while (st.hasMoreTokens()) {
      ...
      if (token.equals("&")) {
        ...
        // check whether TODO rewrite such that itfilter indexToVariable are definitely used in where indexToVariable
  ...
  private List<int[]> internalizeWhere(ArrayList<ArrayList<String>> whereClauses,
    ...
    for (int j = 0; j < whereClauses.size(); j++) {
        ...
        if (RuleStore.isVariable(elem)) {
          ...
          if (this.tupleStore.isConstant(elem)) {
            ...
            if (null == (r = QRelationFactory.getRelation(elem, id1, id2, clause.size()))) {
              if (verbose) {
                LogMF.info(LOGGER,"unknown relation in WHERE clause: {0}" ,elem );
              }
            ...
            if (verbose) {
              LogMF.info(LOGGER,"unknown constant in WHERE clause: {0}",elem);
            }
      ...
      LogMF.debug(LOGGER,"Relations to be rewritten: {0}",relationsToBeRewritten);
      LogMF.debug(LOGGER,"CreatePattern: {0}",createPattern);
  ...
  /**
   * maps surface FILTER form to internal representation, stored in varvarIneqs and varconstIneqs
   * (as used by Calc.restrict) and predicates; <p> NOTE: code does NOT work for NoValue in queries
   * (FILTER), but DOES work for @test in rules SOLUTION: replace QueryParseException by a warning
   */
        ...
        for (int i = 1; i < filter.size(); i++) {
          ...
          if (RuleStore.isVariable(next)) {
            ...
            if (this.tupleStore.isConstant(next)) {
              ...
              if (verbose) {
                LogMF.info(LOGGER,"unknown constant in FILTER predicate: {0}",next );
              }
              tupleStore.putObject(next);
              id = this.tupleStore.objectToId.get(next);
              args.add(id);
            }
          }
      ...
      // an in-eq constraint; make distinction here: either var-var or var-const
        ...
        if (RuleStore.isVariable(next)) {
          ...
          if (this.tupleStore.isConstant(next)) {
            ...
            if (verbose) {
              LogMF.info(LOGGER,"unknown constant in FILTER in-eq constraint: {0}" ,next );
            }
  ...
  /**
   * maps surface AGGREGATE form to internal representation, stored in aggregates; aggregate list is
   * of the following form: <var1> ... <varN> = <aggregate> <token1> ... <tokenM>, where N >= 1 (at
   * least _one_ var) and M >= 0 (potentially _no_ args)
   */
  private void internalizeAggregate(ArrayList<ArrayList<String>> aggregateClauses,
    ...
    for (ArrayList<String> clause : aggregateClauses) {
      ...
      // read LHS indexToVariable and check whether LHS indexToVariable are brand new
      ...
      for (int i = 0; i < eqpos; i++) {
        ...
        // assign new ids (= negative ints); note: LHS aggregate indexToVariable might appear more than once
      ...
      // read remaining args (indexToVariable, URIs, XSD atoms) and store internal id
      args = new int[clause.size() - eqpos - 2];
      for (int i = eqpos + 2; i < clause.size(); i++) {
        elem = clause.get(i);
        // a variable
        if (RuleStore.isVariable(elem)) {
          // check whether projectedVars ontologyContainsTuple "*"; if so, aggregate indexToVariable must be contained
          // in foundVars; otherwise, aggregate indexToVariable must be contained in projectedVars
          if (projectedVars.contains("*")) {
            if (!foundVars.contains(elem)) {
              throw new QueryParseException(
                  "  aggregate RHS variable not contained in SELECT: " + elem);
            }
          } else {
        ...
        // a constant
          ...
          if (this.tupleStore.isConstant(elem)) {
            ...
            if (verbose) {
              LogMF.info(LOGGER,"unknown constant in AGGREGATE function: {0}",  elem );
            }
  ...
  /**
   * queries the index for each clause and {@link IndexLookup}, and then
   * successively joins the results
   */
  private BindingTable queryAndJoin(ArrayList<int[]> patterns,
      ArrayList<Table> tables, HashSet<IndexLookup> lkps) {
    LogMF.debug(LOGGER,"Performing query and join", null);
    // call queryIndex for each pattern/table combination in order to construct binding tables
    ArrayList<BindingTable> localTables = new ArrayList<BindingTable>();
    BindingTable bt;
    // perform index lookups
    for (IndexLookup lkp : lkps) {
      LogMF.debug(LOGGER,"Perform Index lookup", null);
      bt = new BindingTable();
      lkp.apply(bt);
      localTables.add(bt);
    }
    ...
    for (int i = 0; i < patterns.size(); i++) {
      LogMF.info(LOGGER,"Perform regular lookup {0}" , Arrays.toString(patterns.get(i)));
      bt = new BindingTable();
      bt.table = tupleStore.queryIndex(patterns.get(i), tables.get(i));
  ...
  /**
   * potentially modifies the binding table, due to the use of DISTINCT and the set of projected
   * variables
   */
  private void makeDistinctAndProject(BindingTable bt,
    ...
    // now distinguish between "*" and list of indexToVariable in SELECT
    if (projectedVars.contains("*")) {
      if (makeDistinct)
      ...
      // note: non-projected indexToVariable need to be removed from bt's nameToPos and nameToExternalName fields
  ...
  /**
   * applies the aggregates as specified in the AGGREGATE section and joins the resulting tables (if
   * there is more than one aggregate)
   *
   * @see de.dfki.lt.hfc.Calc.join()
   */
  private BindingTable aggregateAndJoin(BindingTable bt, ArrayList<Aggregate> aggregates) {
    LogMF.debug(LOGGER,"Performing aggregate and join", null);
    // NOTE: each aggregate is required to provide the right nameToPos and nameToExternalName mappings
    BindingTable result = applyAggregate(aggregates.get(0),
        bt);  // one aggregate is guaranteed to exist
    BindingTable next;
  ...
  /**
   * potentially renames internal variable names (negative ints) in next, given result; this
   * requires changing next.nameToPos and next.nameToExternalName
   */
  ...
  private BindingTable applyAggregate(Aggregate aggregate, BindingTable oldTable) {
    // create a new binding table from parameter oldTable that ontologyContainsTuple the variable
